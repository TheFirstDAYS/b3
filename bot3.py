...
''', reply_markup=keyboard)
    ...
    if current_time - provided_time > required_duration:
        ...
        contact_button = types.InlineKeyboardButton(text="✨ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 ✨", url="https://t.me/NoMoreBins")
        keyboard.add(contact_button, ahmed)
        bot.send_message(chat_id=message.chat.id, text=f'''YOUR SUBSCRIPTION HAS BEEN EXPIRED!</b>
        ''', reply_markup=keyboard)
        with open('data.json', 'r') as file:
            json_data = json.load(file)
            ...
            with open("combo.txt", 'r') as file:
                ...
                for cc in lino:
                    if stopuser[f'{id}']['status'] == 'stop':
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=' MAGIC STOPPED ✅\nDEV ➜ @stripe_xD')
                        return
                    try:
...
- - - - - - - - - - - - - - - - - - - - - - -
◆ 𝑩𝒀: @stripe_xD
◆ 𝑻𝑨𝑲𝑬𝑵 ➜ {"{:.1f}".format(execution_time)} seconds .</b>'''
                    msgc = f'''<b>𝑪𝑪𝑵 ☑️
            - - - - - - - - - - - - - - - - - - - - - - -
◆ 𝑪𝑨𝑹𝑫 ➜ <code>{cc}</code>
◆ 𝑮𝑨𝑻𝑬𝑾𝑨𝒀 ➜ {gate}
...
❆═══» 𝙸𝙽𝙵𝙊 «═══❆
｢𝙱𝙸𝙽」➔ <code>{cc[:6]}</code>
｢𝙸𝙽𝙵𝙊」➔ <code>{brand} - {card_type} - {level}</code>
｢𝙱𝙰𝙽𝙺」➔ <code>{bank}</code>
...
''', reply_markup=keyboard)
    ...
    if current_time - provided_time > required_duration:
        ...
        ahmed = types.InlineKeyboardButton(text="✨ CHANNEL ", url="https://t.me/NoMoreBins")
        keyboard.add(contact_button, ahmed)
        bot.send_message(chat_id=message.chat.id, text=f'''<b>OHH ! IT SEEMS LIKE YOUR SUBSCRIPTION HAS EXPIRED.</b>
    ''', reply_markup=keyboard)
        with open('data.json', 'r') as file:
            json_data = json.load(file)
        json_data[str(id)]['timer'] = 'none'
